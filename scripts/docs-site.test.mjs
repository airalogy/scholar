...
test('the repository defaults to English with a Chinese README alternative', async () => {
  const [englishReadme, chineseReadme] = await Promise.all([
    readRepositoryFile('README.md'),
    readRepositoryFile('README.zh-CN.md'),
  ])

  assert.match(englishReadme, /^# Airalogy Scholar\n\nEnglish \| \[简体中文\]/u)
  assert.match(chineseReadme, /^# Airalogy Scholar\n\n\[English\].*简体中文/u)
})
