<template>
  <section class="identity-panel">
    ...
    <div v-if="detail && selected" class="person-detail">
      ...
      <a-textarea
        v-model="notes"
        :textarea-attrs="{ id: 'identity-admin-notes' }"
        :disabled="saving"
        :max-length="2000"
      ...
      <ul class="identifier-list">
        <li v-for="identifier in detail.identifiers" :key="identifier.id">
          <div>
            ...
            <span v-if="identifier.isPrimary">{{ $t('identity.primary') }}</span
            ><small>{{
              identifier.revokedAt ? $t('identity.revoked') : $t('identity.active')
            }}</small>
          </div>
          <a-popconfirm
            v-if="!identifier.revokedAt"
            :content="$t('identity.revokeConfirm')"
            @ok="revoke(identifier.id)"
          >
            <a-button status="danger" size="small" :disabled="!canChange">{{
              $t('identity.revoke')
            }}</a-button>
          </a-popconfirm>
          <a-button v-else size="small" :disabled="!canChange" @click="add(identifier.value)">{{
            $t('identity.restore')
          }}</a-button>
        </li>
      </ul>
      <form class="add-identifier" @submit.prevent="add(value)">
        ...
        <a-input
          v-model="value"
          :input-attrs="{ id: 'identity-add-value', autocomplete: 'off' }"
          :disabled="saving"
          :max-length="100"
        />
        <a-button
          html-type="submit"
          type="primary"
          :disabled="!canChange || !value.trim()"
          :loading="saving"
          >{{ $t('identity.addVerified') }}</a-button
        >
      </form>
...
const canChange = computed(
  () => !saving.value && !loading.value && Boolean(notes.value.trim()) && confirmed.value,
)
...
const add = async (identifier: string): Promise<void> => {
  ...
  await mutate((signal) =>
    addIdentityIdentifier(props.slug, id, identifier.trim(), notes.value.trim(), signal),
  )
}
const revoke = async (identifierId: string): Promise<void> => {
  ...
  if (!id) return
  await mutate((signal) =>
    revokeIdentityIdentifier(props.slug, id, identifierId, notes.value.trim(), signal),
  )
}
watch(
  () => props.slug,
