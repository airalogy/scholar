<template>
  <section class="identity-review-panel">
    <header>
      <h2>{{ $t('identity.queueTitle') }}</h2>
      <a-button :disabled="saving" :loading="loading" @click="loadList">{{
        $t('common.refresh')
      }}</a-button>
    </header>
    ...
    <ul class="identity-request-list">
      <li v-for="item in items" :key="item.id">
        <button
          type="button"
          :aria-pressed="detail?.request.id === item.id"
          :disabled="saving"
          @click="selectRequest(item.id)"
        >
          <span
            ><strong>{{ item.name }}</strong> · {{ item.internalId }}</span
          >
          <small
            >{{ $t(`identity.status.${item.status}`) }} ·
            {{ new Date(item.createdAt).toLocaleString(locale) }}</small
          >
        </button>
      </li>
    </ul>
    ...
    <div v-if="detail" class="identity-review-detail">
      ...
      <dl>
        <dt>{{ $t('identity.ssoName') }}</dt>
        <dd>{{ detail.request.name }}</dd>
        <dt>{{ $t('identity.ssoEmail') }}</dt>
        <dd>{{ detail.request.email }}</dd>
        <dt>{{ $t('identity.previousId') }}</dt>
        <dd>{{ detail.request.previousInternalId }}</dd>
        <dt>{{ $t('identity.explanation') }}</dt>
        <dd>{{ detail.request.explanation }}</dd>
      </dl>
      <template v-if="isOpen">
        <h4>{{ $t('identity.chooseVerifiedPerson') }}</h4>
        <p>{{ $t('identity.adminHint') }}</p>
        <IdentityPersonPicker
          :slug="slug"
          ...
          @select="target = $event"
        />
        <p v-if="target" class="selected-person">
          {{ $t('identity.selectedTarget') }}：{{ target.name }} · {{ target.internalId }}
        </p>
        <label for="identity-review-notes">{{ $t('identity.verificationNotes') }}</label>
        <a-textarea
          v-model="notes"
          :textarea-attrs="{ id: 'identity-review-notes' }"
          :max-length="2000"
          :disabled="saving"
          :auto-size="{ minRows: 2, maxRows: 6 }"
        />
        <a-checkbox v-model="confirmed" :disabled="saving">{{
          $t('identity.adminConfirmed')
        }}</a-checkbox>
        <label for="identity-applicant-message">{{ $t('identity.applicantMessage') }}</label>
        <a-textarea
          v-model="message"
          :textarea-attrs="{ id: 'identity-applicant-message' }"
          :max-length="1000"
          :disabled="saving"
        ...
        <div class="review-actions">
          <a-popconfirm :content="$t('identity.approveConfirm')" @ok="decide('approve')">
            <a-button type="primary" :disabled="!canApprove" :loading="saving">{{
              $t('identity.approve')
            }}</a-button>
          </a-popconfirm>
          <a-button :disabled="!canRespond" @click="decide('request_information')">{{
            $t('identity.requestInformation')
          }}</a-button>
...
const isOpen = computed(
  () => detail.value && ['pending', 'needs_information'].includes(detail.value.request.status),
)
const canApprove = computed(
  () =>
    isOpen.value &&
    !saving.value &&
    Boolean(notes.value.trim()) &&
    confirmed.value &&
    Boolean(target.value?.userId),
)
const canRespond = computed(
  () =>
    isOpen.value && !saving.value && Boolean(notes.value.trim()) && Boolean(message.value.trim()),
)
...
const loadList = async (): Promise<void> => {
  ...
  loading.value = true
  try {
    const result = await listIdentityRequests(
      props.slug,
...
const decide = async (action: 'approve' | 'reject' | 'request_information'): Promise<void> => {
  ...
  errorKey.value = ''
  try {
    await decideIdentityRequest(
      props.slug,
      id,
      {
        action,
        personId: action === 'approve' ? target.value?.id : undefined,
        notes: notes.value.trim(),
        applicantMessage: message.value.trim() || undefined,
      },
      request.signal,
    )
    if (request.signal.aborted) return
    ...
    if (!request.signal.aborted) saving.value = false
  }
}
...
.identity-review-panel h2 {
  margin: 0;
  font-size: 18px;
}
.identity-review-panel select {
  margin-left: 12px;
  min-height: 36px;
  border: 1px solid var(--color-border-2);
  padding: 4px 12px;
...
.identity-request-list button[aria-pressed='true'] {
  border-color: rgb(var(--primary-6));
}
.identity-request-list small {
...
.identity-review-detail dd {
  margin: 4px 0 16px;
  white-space: pre-wrap;
  overflow-wrap: anywhere;
}
.identity-review-detail p {
  line-height: 1.6;
}
.selected-person {
  padding: 12px;
