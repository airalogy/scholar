...
Thank you for helping improve Scholar. Before opening a pull request:
...
4. Run `pnpm install` once to enable the repository's pre-push hook. It validates the GitHub Actions toolchain configuration and runs `pnpm check` before every push.
5. Add or update both `CHANGELOG.md` and `CHANGELOG.zh-CN.md` for user-visible changes.
6. Sign off each commit with `git commit -s` to certify the [Developer Certificate of Origin](https://developercertificate.org/).
